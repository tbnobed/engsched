...
- **Calendar & Scheduling**: Weekly calendar view with user-specific timezone support, technician schedule management, location-based filtering, time-off tracking (including all-day OOO), and conflict prevention for scheduling. Supports 30-minute increments. Schedule form uses three-mode tab system (Single Day, Multiple Days, Date Range) with quick-select buttons (This Week, Next Week, Next 2 Weeks, This Month) and day-of-week chip picker for range mode. Bulk select mode allows selecting multiple schedules and deleting them at once via a floating action bar (available on both main calendar and personal schedule pages). Schedule modal uses static backdrop to prevent accidental closure when using date pickers.
...
- **Dashboard ("Command Center" design)**: Dark-themed grid layout with live clock, left technicians rail (status pills: On/Off/OOO with current location and shift end time), 4 KPI cards (Active Tickets, High Priority, On Shift, Bookings Today), center live timeline showing each technician's day as colored blocks (auto-fits to actual schedule range, defaults 8:00–20:00), bottom Active Tickets list with priority badges and unread-activity dots, right Studio Bookings panel that loads from `/api/studio-bookings`. Auto-refreshes tickets every 30s and bookings every 3min. Clicking a technician filters the calendar by that user; clicking a ticket opens its detail page.
